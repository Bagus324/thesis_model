    ...
    def load_data(self, type):
        if type == "wops":
            ...
            if self.sastrawi == True and os.path.exists(self.pickle_file_path):
                ...
                data = pd.read_csv('datasets/df_with_positif_undersam.csv', sep=",", encoding="ISO-8859-1")
                data = data.dropna(how="any")
                tweet = data
                ...
                if self.sastrawi:
                    ...
                    tweet["Tweet"] = tweet["Tweet"].progress_apply(lambda x: self.stemmer_text(x))
                    # Save the data to a new pickle file
                    with open(self.pickle_file_path, 'wb') as pickle_file:
                        pickle.dump(tweet, pickle_file)
                    print(f"Preprocessed data saved to {self.pickle_file_path}")
            tweet = tweet.sample(frac=1)
            tweet = tweet.sample(frac=1)
            tweet = tweet.sample(frac=1)
